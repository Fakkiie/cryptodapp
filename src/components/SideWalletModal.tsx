...
export default function SideWalletModal({
	...
	const [walletTotalDollarBalance, setWalletTotalDollarBalance] =
		useState<number>(0);
	...
	useEffect(() => {
		...
		const getWalletInfo = async () => {
			const { solBalance, totalDollarBalance } = await getAccountBalance({
				publicKey,
				connection: endpoint.connection,
			});

			setWalletSolBalance(solBalance);
			setWalletTotalDollarBalance(totalDollarBalance);
		};
		getWalletInfo();
	}, [loggedIn, publicKey]);

	// const getSolDollarValue = async () => {
	// 	if (!publicKey) {
	// 		return;
	// 	}
	// }
				...
				<div className='px-6 pb-6 pt-2 bg-black/10 flex justify-between items-center'>
					<div className='flex flex-col'>
						<h1 className='text-3xl font-bold'>
							${walletTotalDollarBalance.toFixed(2)}
						</h1>
						<p className='text-gray-500'>
							~{walletSolBalance.toFixed(8)} SOL
						</p>
					</div>
